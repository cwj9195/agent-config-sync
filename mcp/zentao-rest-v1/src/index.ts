...
import { z } from "zod";
import { ZentaoRestV1API } from './zentaoRestV1Api.js';
import { loadConfig, saveConfig, ZentaoConfig } from './config.js';
import { BugResolution, BugStatus, TaskStatus, TaskUpdate, StoryStatus, TestCaseStatus, CreateTestCaseRequest, TestRunRequest, BugUpdate } from './types.js';
...
const server = new McpServer({
    name: "Zentao REST v1 with Legacy Fallback",
    version: "2.0.0-rest-v1"
});

// 初始化 REST v1 优先、Legacy 兜底的双通道 API 实例。
let zentaoApi: ZentaoRestV1API | null = null;
...
async function ensureInitialized(): Promise<void> {
    ...
    zentaoApi = new ZentaoRestV1API(config);
}
...
// Add createTask tool - 使用 REST v1 在指定执行下创建日常任务
server.tool("createTask",
    {
        ...
        estimate: z.number().finite().nonnegative().optional(),
        estStarted: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(),
    ...
    async ({ executionId, name, type, assignedTo, estimate, estStarted, deadline, pri, desc, module, story, parent }) => {
        await ensureInitialized();
        try {
            // 底层适配器会按 REST v1 优先、Legacy 兜底策略创建并回读任务。
...
// Add updateBug tool - 使用 REST v1 更新 Bug 并回读核验
server.tool("updateBug",
    {
        ...
        update: z.object({
            ...
            message: "update 至少需要一个字段"
        })
    },
    async ({ bugId, update }) => {
        await ensureInitialized();
        try {
            // 底层适配器会先尝试 REST v1，路由不兼容时才回退 Legacy。
            const bug = await zentaoApi!.updateBug(bugId, update as BugUpdate);
            return {
                content: [{
                    type: "text",
                    text: JSON.stringify({
            ...
            throw createError(
                ErrorCode.API_ERROR,
                `更新 Bug ${bugId} 失败: ${error instanceof Error ? error.message : String(error)}`,
                undefined,
                error
            );
        }
