...
import { Bug, BugStatus, Task, TaskStatus, Story, StoryStatus, TestCase, TestCaseStatus, TestResult, TestTask, CreateTestCaseRequest, TestRunRequest, Product, TaskUpdate, BugResolution, CreateTaskRequest, BugUpdate } from './types.js';
...
export class ZentaoLegacyAPI {
     ...
     */
    private extractCreatedTaskId(payload: unknown): number | undefined {
        ...
        while (pending.length > 0) {
            ...
            for (const key of ['task', 'data', 'result', 'load']) {
                if (record[key] !== undefined && record[key] !== null) pending.push(record[key]);
            }
        }
    ...
    async createTask(request: CreateTaskRequest): Promise<Task> {
        ...
        // 按页面字段名提交单人负责人。
        if (request.assignedTo) body['assignedTo[]'] = request.assignedTo;
        // 仅在用户显式指定时提交可选关联字段。
        if (request.module !== undefined) body.module = request.module;
        if (request.story !== undefined) body.story = request.story;
        if (request.parent !== undefined) body.parent = request.parent;

        // Legacy 写请求只发送一次。
        const response = await this.postRequest<any>(url, body);
        // 没有任务 ID 时不能假定创建成功。
