...
class RestV1FallbackError extends Error {
     ...
     */
    constructor(message, safeToFallback) {
        super(message);
...
class RestV1Client {
    ...
    isSafeFallbackError(error) {
        if (error instanceof RestV1FallbackError)
     ...
     * @returns 服务端 JSON 数据
     */
    async request(request, retryAuth = true) {
        // 获取当前请求使用的 Token。
    ...
    async getBugDetail(bugId) {
        ...
        const data = await this.request({ method: 'GET', url: `/bugs/${bugId}` });
        // 校验并转换 Bug ID。
        const id = Number(data.id);
        if (!Number.isInteger(id) || id <= 0)
            throw new Error(`REST v1 Bug ${bugId} 详情缺少 id`);
    ...
    async getTaskDetail(taskId) {
        ...
        const data = await this.request({ method: 'GET', url: `/tasks/${taskId}` });
        // 校验并转换任务 ID。
        const id = Number(data.id);
        if (!Number.isInteger(id) || id <= 0)
            throw new Error(`REST v1 任务 ${taskId} 详情缺少 id`);
    ...
    async createTask(request) {
        ...
        try {
            ...
            const response = await this.request({ method: 'POST', url: `/executions/${request.executionId}/tasks`, data: body }, false);
            ...
            // 校验回读标题，避免误认其他任务。
            if (task.name !== request.name.trim())
                throw new Error(`REST v1 创建任务回读名称不一致: ${taskId}`);
            // 如果返回执行 ID，则校验任务归属。
            ...
            if (this.isSafeFallbackError(error))
                throw error;
            throw error;
        }
    }
    ...
    async updateBug(bugId, update) {
        ...
        const current = await this.request({ method: 'GET', url: `/bugs/${bugId}` });
        ...
        await this.request({ method: 'PUT', url: `/bugs/${bugId}`, data: body }, false);
    ...
    async resolveBug(bugId, resolution) {
        ...
        // 重复 Bug 仅在用户明确提供时提交。
        if (resolution.duplicateBug)
            body.duplicateBug = resolution.duplicateBug;
        // REST v1 专用解决接口只提交一次。
        await this.request({ method: 'POST', url: `/bugs/${bugId}/resolve`, data: body }, false);
    }
    /**
     * 更新任务。
     * @param taskId 任务 ID
    ...
    async updateTask(taskId, update) {
        ...
        // REST v1 写入只提交一次。
        await this.request({ method: 'PUT', url: `/tasks/${taskId}`, data: body }, false);
        // 写后回读任务详情。
    ...
    async finishTask(taskId, update) {
        ...
        // REST v1 完成操作只提交一次。
        await this.request({ method: 'POST', url: `/tasks/${taskId}/finish`, data: body }, false);
    }
}
/**
...
export class ZentaoRestV1API extends ZentaoLegacyAPI {
     ...
     * @returns 首个成功读取结果
     */
    async readWithFallback(label, restReader, legacyReader) {
        try {
