...
export class ZentaoLegacyAPI {
     ...
     * @returns 是否需要重新认证
     */
    isAuthenticationFailure(response) {
     ...
     */
    async request(url, params, retried = false, validate, responseDescription = url) {
        ...
        try {
            ...
            if (this.isSessionExpired(response.data) || this.isAuthenticationFailure(response)) {
                if (!retried) {
                    ...
                    return this.request(url, params, true, validate, responseDescription);
                }
                throw new Error('会话已过期，重新登录后仍然失败');
            }
            const data = this.parseResponseData(response.data, url);
     ...
     */
    async getProducts() {
        const data = await this.request('/product-index-no.json', undefined, false, value => this.hasResponseField(value, 'products'), '产品列表');
     ...
     */
    async getMyTasks() {
        const data = await this.request('/my-task.json', undefined, false, value => this.hasResponseField(value, 'tasks'), '我的任务列表');
    ...
    /**
     * 从 Legacy 写入响应中提取新任务 ID。
     * @param payload 禅道写入接口返回的数据
     * @returns 新任务 ID，无法提取时返回 undefined
     */
    extractCreatedTaskId(payload) {
        // 使用待检查队列兼容 data、task、result 和 load 等嵌套结构。
        ...
        while (pending.length > 0) {
            ...
            if (typeof current === 'string') {
                // 从跳转地址中提取任务编号。
                const match = current.match(/task-(?:view|edit|create)-([0-9]+)/);
                if (match) {
                    // 将地址中的任务编号转换为正整数。
            ...
            // 继续检查常见嵌套响应节点。
            for (const key of ['task', 'data', 'result', 'load']) {
    ...
    /**
     * 通过 Legacy 页面接口创建执行下的任务，供 REST v1 不可用时兜底。
     * @param request 创建任务字段
     * @returns 已回读的任务详情
     */
    async createTask(request) {
        // 校验执行 ID，避免提交无效的创建路径。
        if (!Number.isInteger(request.executionId) || request.executionId <= 0) {
            throw new Error('创建任务失败：executionId 必须是正整数');
        }
        // 校验任务名称，避免创建空标题任务。
        if (!request.name.trim()) {
            throw new Error('创建任务失败：name 不能为空');
        }
        // 校验预估工时，允许零和小数。
        if (request.estimate !== undefined && (!Number.isFinite(request.estimate) || request.estimate < 0)) {
            throw new Error('创建任务失败：estimate 必须是非负数字');
        }
        // 校验优先级范围。
        if (request.pri !== undefined && (!Number.isInteger(request.pri) || request.pri < 1 || request.pri > 4)) {
            throw new Error('创建任务失败：pri 必须是 1 到 4 的整数');
        }
        // 任务创建页面使用 execution-story-module 组成 Legacy 路径。
        const storyId = request.story ?? 0;
        // 未指定模块时使用页面默认模块值 0。
        const moduleId = request.module ?? 0;
        // 组装当前禅道页面实际使用的 JSON 路径。
        const url = `/task-create-${request.executionId}-${storyId}-${moduleId}.json`;
        // 只提交已确定的业务字段。
        const body = {
        ...
        if (request.story !== undefined)
            body.story = request.story;
        if (request.parent !== undefined)
            body.parent = request.parent;
        // Legacy 写请求只发送一次。
        const response = await this.postRequest(url, body);
        // 没有任务 ID 时不能假定创建成功。
        const taskId = this.extractCreatedTaskId(response);
        if (!taskId) {
            throw new Error('创建任务失败：写入响应未返回可核验的任务 ID');
        }
        // 回读任务详情确认任务已落库。
        const createdTask = await this.getTaskDetail(taskId);
        // 校验回读标题，避免误认其他任务。
        if (createdTask.name !== request.name.trim()) {
            throw new Error(`创建任务回读失败：任务 ${taskId} 名称与提交值不一致`);
        }
        return createdTask;
    }
    /**
     * 通过 Legacy 页面接口更新 Bug，供 REST v1 不可用时兜底。
     * @param bugId Bug ID
    ...
    async updateBug(bugId, update) {
        ...
        if (!Number.isInteger(bugId) || bugId <= 0) {
            throw new Error('更新 Bug 失败：bugId 必须是正整数');
        }
        // 只提交非空且用户明确指定的字段。
        const body = {};
        ...
        if (Object.keys(body).length === 0) {
            throw new Error('更新 Bug 失败：没有可提交的字段');
        }
        // Legacy 写请求只发送一次。
        await this.postRequest(`/bug-edit-${bugId}.json`, body);
        // 回读 Bug 详情确认更新结果。
     ...
     */
    async getMyBugs() {
        const data = await this.request('/my-bug.json', undefined, false, value => this.hasResponseField(value, 'bugs'), '我的 Bug 列表');
    ...
    async getProductBugs(productId, status, moduleId) {
        try {
            ...
            while (hasMore) {
                ...
                const data = await this.request(url, undefined, false, value => this.hasResponseField(value, 'bugs'), `产品 ${productId} 的 Bug 列表`);
                const bugs = data.bugs || {};
    ...
    async getBugDetail(bugId) {
        const data = await this.request(`/bug-view-${bugId}.json`, undefined, false, value => Boolean(value?.bug?.id != null), `Bug ${bugId} 详情`);
        const bug = data.bug;
