...
server.tool("createTask", {
    ...
    try {
        ...
        throw createError(ErrorCode.API_ERROR, `创建任务失败: ${error instanceof Error ? error.message : String(error)}`, undefined, error);
    }
});
...
server.tool("updateBug", {
    ...
    try {
        ...
        const bug = await zentaoApi.updateBug(bugId, update);
        return {
            content: [{
                    type: "text",
                    text: JSON.stringify({
        ...
        throw createError(ErrorCode.API_ERROR, `更新 Bug ${bugId} 失败: ${error instanceof Error ? error.message : String(error)}`, undefined, error);
    }
});
// Add updateTask tool
