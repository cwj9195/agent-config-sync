...
# zentao-rest-v1-local

这是独立的 REST v1 优先、Legacy 兜底禅道 MCP。

## 本地维护范围

- 保留原有工具名和参数，避免上层 `zentao-mcp-tasks` 重新适配。
- 优先使用禅道 RESTful API v1，使用 Token 认证。
- REST v1 路由明确不兼容时回退到 Legacy Session API。
...
- `createTask` - 在指定执行下创建日常任务，REST v1 不可用时回退 Legacy。
- `updateBug` - 更新 Bug 并回读核验。
- `getBugDetail` / `getTaskDetail` - REST v1 优先读取，Legacy 兜底。
