 ...
 * 格式化 Bug 为 Markdown
 */
export function formatBugAsMarkdown(bug: Bug): string {
    const lines: string[] = [];
 ...
 * 生成需求摘要
 */
export function generateStorySummary(story: Story): string {
    const parts: string[] = [];
 ...
 * 生成 Bug 摘要
 */
export function generateBugSummary(bug: Bug): string {
    const parts: string[] = [];
